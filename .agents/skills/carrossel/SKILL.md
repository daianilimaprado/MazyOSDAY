...
description: >
  ...
  Suporta carrossel texto puro, carrossel com foto IA e post único.
...
- **Geração de Imagens (opcional):** ferramenta nativa de geração de imagem ou script `scripts/gerar-imagem.js`
...
- **Foto:** gerada por IA ou real (passada pelo usuário)
...
1. Montar prompt em inglês (a geração funciona melhor em inglês)
...
3. Gerar a imagem e salvar em `marketing/conteudo/<pasta>/foto-<nome>.png`.
...
2. Criar `render.js` na mesma pasta — script Node com Playwright que abre o HTML e tira screenshot de cada `.slide` em 1080x1350. Pode reutilizar `node_modules` de uma pasta anterior:
```bash
