...
description: >
  ...
  `_memoria/estrategia.md`, `identidade/design-guide.md` e adapta o `GEMINI.md` e `CLAUDE.md` conforme o perfil.
...
Conferir o nome da pasta atual (`basename "$(pwd)"`). Se for `mazyos`, `MazyOS`, `MazyOS-main`, `mazyos-main`, `MazyOSDAY` ou variação genérica:

> "Notei que a pasta atual ainda tem nome genérico ('<nome-atual>'). O ideal é a pasta ter o nome do seu negócio, não 'MazyOS'. Quando terminarmos o setup, te lembro de renomear (é rápido — fechar a IDE, renomear a pasta no Finder/Explorer, abrir de novo). Bora seguir?"
...
A resposta determina qual template de `GEMINI.md` e `CLAUDE.md` aplicar (ver `templates/perfis/`).
...
Pegar o template correspondente ao perfil escolhido na Fase 1 (`templates/perfis/gemini-md-<perfil>.md` ou `templates/perfis/claude-md-<perfil>.md`), adaptar com o nome do negócio e estrutura de pastas mencionada nas respostas, e atualizar os arquivos da raiz.
...
✓ GEMINI.md e CLAUDE.md adaptados pro perfil [perfil]
```
...
> 3. Abre a IDE de novo na pasta renomeada
>
