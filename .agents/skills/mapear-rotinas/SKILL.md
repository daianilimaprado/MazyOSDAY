...
Ler `templates/skills/catalogo.md` pra ver se alguma das tarefas mencionadas já é coberta por uma skill nativa ou validada pelo MazyOS. Se sim, sugerir a skill existente em vez de criar uma nova:
...
1. Criar pasta `.agents/skills/<nome>/` (e opcionalmente espelhar em `.claude/skills/<nome>/`)
2. Criar `SKILL.md` com:
   - Frontmatter: `name`, `description` (descreve em 3ª pessoa quando deve ser invocada)
...
Pra usar: digita / e o nome da skill em qualquer sessão ou descreva a tarefa naturalmente.
...
- Cada skill criada precisa ter um trigger claro (`description` precisa indicar claramente quando invocar) — sem isso a skill não é ativada no Progressive Disclosure
- Se a skill depender de uma ferramenta que o usuário não tem (ex: API do Notion sem conector configurado), avisar antes de criar e oferecer a versão simplificada
