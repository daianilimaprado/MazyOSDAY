...
name: novo-projeto
description: >
  Cria uma pasta de projeto nova com `GEMINI.md` e `CLAUDE.md` dedicados, depois de uma entrevista curta sobre
...
Quando o usuário começa um projeto novo (cliente, iniciativa, produto), cria uma pasta com `GEMINI.md` e `CLAUDE.md` próprios que herdam contexto da raiz e adicionam o que é específico do projeto.
...
- **Cliente novo:** criar em `clientes/<Nome>/` (ou na pasta equivalente do perfil — ler `GEMINI.md` da raiz pra confirmar a convenção)
...
- `GEMINI.md` e `CLAUDE.md` do projeto (instruções herdadas + específicas)
...
### Passo 4 — Conteúdo do `GEMINI.md` do projeto
...
✓ GEMINI.md e CLAUDE.md do projeto
✓ briefing.md
✓ Subpastas: [lista]

Quando for trabalhar nesse projeto, as regras específicas desta pasta serão carregadas automaticamente junto com as da raiz.
```
